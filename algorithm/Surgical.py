...
class SurgicalServer(BaseServer):
    ...
    def __init__(self, train_datasets, test_datasets, args):
        ...
        # 修复：根据模型类型适配surgical的mode列表
        if args.model == 'vit':
            # ViT模型支持的mode（匹配ViT的surgical方法）
            self.supported_modes = ['encoder', 'last_layer', 'patch_embed', 'attention', 'mlp']
        else:
            # CNN/ResNet模型的原有mode
            self.supported_modes = ['block1', 'block2', 'block3', 'block4', 'last_layer']
    ...
    def learn_to_adapt(self, args):
        ...
        if args.surgical_metric == 'valid':
            ...
            modes = self.supported_modes  # 使用适配后的mode列表
            accs = []
            for mode in modes:
                self.model.load_state_dict(state)
                self.model.surgical(mode)  # 调用ViT/ResNet的surgical方法
                self.model.train()
                ...
                print(f"Mode: {mode}, Accuracy: {agg_metric:.4f}")
                accs.append(agg_metric)

            best_mode = modes[np.argmax(accs)]
            print(f"Selected best mode: {best_mode}")
            self.model.load_state_dict(state)
            self.model.surgical(best_mode)

        else:
            raise NotImplementedError(f"Unsupported surgical metric: {args.surgical_metric}")

    def adapt_and_eval(self, args):
        # current global model
        global_state = deepcopy(self.model.state_dict())  # 修复：兼容ViT模型，移除updated_state_dict()
...
class SurgicalClient(BaseClient):
    ...
    def adapt_one_step(self, model, X, Y, unspv_loss_func, args):
        ...
        # 修复：兼容ViT模型的LayerNorm/StandardLayerNormAdapter
        if hasattr(model, 'set_running_stat_grads'):
            model.set_running_stat_grads()
        else:
            # 对ViT的LayerNorm手动设置梯度
            for m in model.modules():
                if isinstance(m, (nn.LayerNorm, StandardLayerNormAdapter)):
                    if m.requires_grad:
        ...
        for p in model.parameters():
            if p.requires_grad:
                unspv_grad.append(p.grad.clone() if p.grad is not None else torch.zeros_like(p))
        ...
        with torch.no_grad():
            ...
            for p in model.parameters():
                if p.requires_grad:
                    p -= args.lm_lr * unspv_grad[grad_idx]
                    grad_idx += 1
