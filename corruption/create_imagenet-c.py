...
# 设置路径
val_path = "/mnt/sda/PythonProject/Learning_Dataset/ImageNet_val/val"
output_base = "/mnt/sda/PythonProject/Learning_Dataset/ImageNet-C"
...
#     x = x.resize((int(x.size[0] * c), int(x.size[1] * c), Image.BOX)
#     x = x.resize((x.size[0], x.size[1]), Image.BOX)
#     return x
...
def impulse_noise(x, severity=1):
    ...
    x = skutil.random_noise(np.array(x) / 255., mode='s&p', amount=c)
    return np.clip(x, 0, 1) * 255
...
def motion_blur(x, severity=1):
    ...
    if x.shape != (x.shape[0], x.shape[1]):
...
def frost(x, severity=1):
    ...
    try:
        ...
        # 如果找不到frost图片，使用随机噪声代替
        frost = np.random.uniform(0.5, 1.0, size=(x.size[1], x.size[0], 3)) * 255
        frost = frost.astype(np.uint8)
    else:
        frost = cv2.resize(frost, (0, 0), fx=0.2, fy=0.2)
        x_start, y_start = np.random.randint(0, frost.shape[0] - x.size[1]), np.random.randint(0,
                                                                                               frost.shape[1] - x.size[
                                                                                                   0])
        frost = frost[x_start:x_start + x.size[1], y_start:y_start + x.size[0]][..., [2, 1, 0]]
...
# ========== 主处理逻辑 ==========
def process_image(image_path, output_dir, dist_name, dist_func, severity):
    try:
        img = Image.open(image_path).convert('RGB')

        # 应用损坏函数
        distorted_img = dist_func(img, severity)

        # 确保输出是PIL图像
        if not isinstance(distorted_img, Image.Image):
            if isinstance(distorted_img, np.ndarray):
                distorted_img = Image.fromarray(distorted_img.astype('uint8'))
            else:
                raise ValueError("Invalid distorted image type")

        # 保存图片
        distorted_img.save(output_dir)
    except Exception as e:
        print(f"Error processing {image_path} with {dist_name} severity {severity}: {str(e)}")
...
def main():
    ...
    for img_path in tqdm(image_paths, desc="Processing images"):
        # 获取图片相对路径（保持原始目录结构）
        rel_path = os.path.relpath(img_path, val_path)
        img_name = os.path.basename(img_path)
        class_name = os.path.basename(os.path.dirname(img_path))

        # 对每种损坏类型进行处理
        for dist_name, dist_func in test_distortions:
            # 创建5个严重等级的输出目录
            for severity in range(1, 6):
                ...
                process_image(img_path, output_path, dist_name, dist_func, severity)
